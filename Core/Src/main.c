...
uint32_t saund_buff_0 [BUF_SIZE];
uint32_t saund_buff_1 [BUF_SIZE];
uint8_t buf_flaf;
  ...
  while (1)
  {
	  ...
	  if(empty_buf_flaf == 0){
		  ...
		  ecard.notes_table->exp_flag[note] = 0;
	    	...
	    	if(buf_flaf == 0){
			  ecard.full_buffer(&ecard, saund_buff_1, note, BUF_SIZE);
			}
			else{
			  ecard.full_buffer(&ecard, saund_buff_0, note, BUF_SIZE);
			}
